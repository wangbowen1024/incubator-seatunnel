...
public class CheckConfigUtilTest {
    ...
    public void testCheckAllExists() {
        ...
        Assert.assertEquals(String.format(errorMsg, "k2"), checkResult.getMsg());

        checkResult = checkAllExists(config, "k0", "k1", "k2", "k3", "k4");
        Assert.assertEquals(String.format(errorMsg, "k2,k3,k4"), checkResult.getMsg());
    }
    ...
    public void testCheckAtLeastOneExists() {
        ...
        Assert.assertEquals(String.format(errorMsg, "k3,k2"), checkResult.getMsg());
    }
    ...
    public void testMergeCheckResults() {
        ...
        Assert.assertTrue(finalResult.isSuccess());
        ...
        Assert.assertEquals(String.format(errorMsg1, "k3"), finalResult.getMsg());

        finalResult = mergeCheckResults(checkResult3, checkResult4);
        Assert.assertEquals(String.format(errorMsg1 + "," + errorMsg2, "k3", "k2,k3"), finalResult.getMsg());
    }
